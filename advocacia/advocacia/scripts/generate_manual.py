...
DOCTYPE_DESCRIPTIONS = {
	...
	"Fee Agreement": (
		"Contrato de **honorários** do caso: valor contratado, modo (Diretos ou "
		"Divisão advogada/cliente), parcelas e vencimentos. Ao salvar, sincroniza **Pagamentos**."
	),
	"Service Record": (
		"**Cobrança de serviços** — itens avulsos a faturar (consulta, petição, diligência). "
		"Use **Sincronizar cobrança** para emitir pagamento total ou parcial."
	),
	"Legal Payment": (
		"Registro de recebimento no caixa do escritório. Origem: parcela de **Honorários** "
		"ou **Cobrança de serviços**. Honorários vêm da sync automática; serviços, da cobrança manual."
	),
	"Hearing": (
...
def _render_finance_guide() -> list[str]:
	return [
		...
		"",
		"Dentro de **Honorários**, o modo **Divisão advogada/cliente** reparte o valor entre "
		"advogada e cliente (ex.: sucumbência). Continua sendo honorários contratuais — "
		"não confundir com Cobrança de serviços.",
		"",
...
def generate():
	...
	lines.extend(_render_finance_guide())
	lines.extend(
		[
			"## Fluxos Comuns",
			"",
