		...
		if (frm.doc.status === "Cancelado") {
			...
			const intro =
				frm.doc.tipo_origem === "Atos Advocatícios"
					? __(
							"Pagamento cancelado — registro imutável. Os atos foram liberados; você pode excluir este pagamento se desejar."
					  )
					: __(
							"Pagamento cancelado — registro imutável. A parcela no acordo foi atualizada; você pode excluir este pagamento se desejar."
					  );
...
function configurar_botoes_atos(frm) {
	if (frm.doc.registro_atos) {
		frm.add_custom_button(__("Ver Registro de Atos"), function () {
			frappe.set_route("Form", "Registro de Atos", frm.doc.registro_atos);
		});
	}

	if (["Pendente", "Vencido"].includes(frm.doc.status)) {
		frm.add_custom_button(__("Cancelar Pagamento"), function () {
			frappe.confirm(
				__(
				...
				function () {
					frappe.call({
						...
						callback: function (r) {
							...
							frappe.show_alert({
								message: __("Pagamento cancelado. Atos liberados."),
								indicator: "green",
							});
							frm.reload_doc();
						},
...
function configurar_botoes_honorarios(frm) {
	if (frm.doc.acordo) {
		frm.add_custom_button(__("Ver Acordo"), function () {
			frappe.set_route("Form", "Acordo de Honorarios Processuais", frm.doc.acordo);
		});
	}
			...
			frappe.confirm(msg_recebido, function () {
				frappe.call({
					...
					callback: function (r) {
						...
						frappe.show_alert({
							message: __("Pagamento cancelado."),
							indicator: "green",
						});
						frm.reload_doc();
					},
