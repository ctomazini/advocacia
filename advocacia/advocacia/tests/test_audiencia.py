...
class TestHearing(FrappeTestCase):
	...
	def test_hearing_presencial_salva(self):
		...
		self.assertEqual(aud.modality, "Presencial")
		self.assertEqual(aud.status or "Agendada", "Agendada")
	...
	def test_status_realizada(self):
		...
		self.assertEqual(aud.status, "Realizada")

	def test_hearing_virtual(self):
		aud = create_test_hearing(modality="Virtual", link_virtual="https://meet.example.com/x")
		self.assertEqual(aud.modality, "Virtual")

	def test_titulo_composto(self):
		servico = create_test_legal_case()
		cliente_nome = frappe.db.get_value("Client", servico.client, "client_name")
		aud = create_test_hearing(servico=servico.name, type="Instrução")
